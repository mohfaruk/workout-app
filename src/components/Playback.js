...
class Playback extends Component {
  ...
  increment = () => {
    this.state.listIndex <
      this.props.selectedExercise[0].exercises.length - 1 &&
      this.setState({
  ...
  render() {
    ...
    const exerciseList = exercises.map((exercise, index) => {
      if (index === this.state.listIndex) {
        return (
          <li key={index} style={selectedExerciseStyle}>
            {exercise}
          </li>
        );
      } else {
    ...
    return (
      <div className="container-fluid" style={playbackBackground}>
        <h2 style={playbackStyle}>{this.props.selectedExercise[0].title}</h2>
        ...
        <hr></hr>
        <ul style={{ backgroundColor: "#fad02c" }}>{exerciseList}</ul>
        <div style={playbackCloseButton}>
          <Button variant="secondary" onClick={this.closePlayback}>
            Close
          </Button>
        </div>
      </div>
